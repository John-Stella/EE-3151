...
Open the `balance.py` script in Notepad++. You will be adding code provided in this section to construct the PD controller. *Note: You must save your work before running the script. You do not need to close the Notepad++ doc between test runs.*
...
Lets now define the angle for the motor to balance around. Note this is not the pendulum angle, but the angle shown on the base of the Qube-Servo 3.
...
**Below the last line of code you added** add another line that mimics the structure of your K Control Gain vector previously defined. This new vector will hold the values read from the Qube-Servo 3 and their derivatives. These two vectors must be the same size so we can use matrix math on them. **Use these names for the vector elements in the provided order:**
...
#### State Variable Adjustments
We need to adjust the elements of the `states` vector to include relative term information from the equation:
...
> The rest of the terms are simply negated in the equation, do this by adding `-` in front of each variable name.
...
We can now calculate the output voltage of the balance control signal. The original voltage control signal can be reconstructed from our vectors by performing the dot product of the two vectors.
